...
// ── Key Schedule: derive 11 round keys from the original 128-bit key ─────────
// Round constant (Rcon) values for rounds 1–10
function [7:0] rcon;
    input [3:0] r;
    case(r)
...
function [31:0] subword;
    ...
    integer j;
    begin
        // Inline S-box lookup for key schedule
        // (uses same lookup table values as sbox.v)
        s0 = sbox_byte(w[31:24]);
        s1 = sbox_byte(w[23:16]);
        s2 = sbox_byte(w[15:8]);
        s3 = sbox_byte(w[7:0]);
        subword = {s0, s1, s2, s3};
    end
...
// Inline S-box for key schedule (same table as sbox.v)
...
// ── Generate 11 round keys (combinational) ───────────────────────────────────
reg [127:0] rk [0:10];
integer r;
...
always @(*) begin
    ...
    for(r = 1; r <= 10; r = r+1) begin
        rk[r][127:96] = rk[r-1][127:96]
                        ^ subword({rk[r-1][23:16], rk[r-1][15:8], rk[r-1][7:0], rk[r-1][31:24]})
                        ^ {rcon(r), 24'h000000};
        rk[r][95:64]  = rk[r-1][95:64]  ^ rk[r][127:96];
        rk[r][63:32]  = rk[r-1][63:32]  ^ rk[r][95:64];
        rk[r][31:0]   = rk[r-1][31:0]   ^ rk[r][63:32];
    end
end

// ── 10-round AES encryption pipeline (combinational rounds, registered result)
wire [127:0] round_in  [0:10];
wire [127:0] round_out [0:9];

// Round 0: initial AddRoundKey
assign round_in[0] = data_in ^ rk[0];

// Rounds 1–9: full round (SubBytes + ShiftRows + MixColumns + AddRoundKey)
genvar g;
generate
    for(g = 0; g < 9; g = g+1) begin : full_rounds
        aes_round rnd(
            .state_in (round_in[g]),
            .round_key(rk[g+1]),
            .state_out(round_out[g])
        );
        assign round_in[g+1] = round_out[g];
    end
endgenerate
...
wire [7:0] fsb [0:15];
generate
    for(g = 0; g < 16; g = g+1) begin : final_sub
        sbox fsbox(.in(round_in[9][127 - g*8 -: 8]), .out(fsb[g]));
    end
endgenerate

// Final ShiftRows (same pattern as aes_round)
wire [7:0] fsr [0:15];
assign fsr[0]=fsb[0];  assign fsr[4]=fsb[4];  assign fsr[8]=fsb[8];  assign fsr[12]=fsb[12];
assign fsr[1]=fsb[5];  assign fsr[5]=fsb[9];  assign fsr[9]=fsb[13]; assign fsr[13]=fsb[1];
assign fsr[2]=fsb[10]; assign fsr[6]=fsb[14]; assign fsr[10]=fsb[2]; assign fsr[14]=fsb[6];
assign fsr[3]=fsb[15]; assign fsr[7]=fsb[3];  assign fsr[11]=fsb[7]; assign fsr[15]=fsb[11];

wire [127:0] final_state = {
    fsr[0], fsr[1], fsr[2],  fsr[3],
    fsr[4], fsr[5], fsr[6],  fsr[7],
    fsr[8], fsr[9], fsr[10], fsr[11],
